...
void print_uart_errors_u() {
    char err[] = "Error: Die Eingabe für unsigned int darf nicht negativ sein" ; 
    int len = strlength(err) ;
    int i = 0 ; 
    ...
    if (test){
        ...
        while (*eingabe != '\0') {
            if (*eingabe != '%') {
                ...
                switch (*eingabe) {
                    case 'c':
                        transmit(va_arg(args, int)); //OKKKKK
                        break;

                    case 's':
                        print_uart(va_arg(args, char *)); //OKKKKK
                        break;

                    case 'x':
                        print_uart_x(va_arg(args, unsigned int), width, zeros); //OKKKK
                        width = 0;
                        zeros = 0;
                        break;

                    case 'i':
                        print_uart_standard(va_arg(args, int), width, zeros); //OKKKKK
                        width = 0;
                        zeros = 0;
                        break;

                    case 'u':
                        negativ = va_arg(args, int);
                        if (negativ < 0) { 
                            print_uart_errors_u();
                            width = 0;
                            zeros = 0;
                            ...
                            print_uart_standard(negativ, width, zeros); //OKKKKK
                            width = 0;
                            zeros = 0;
                            break ;
                        }
