...
int kernelinterrupt = 0 ; 

void set_kernelinterrupt(int ctrl){
  kernelinterrupt = ctrl ; 
}

char isuart = '\0'; 

void set_isuart(char ctrl){
  isuart = ctrl ; 
}
...
int svc_createthread = 0 ; 
void set_svc_createthread(int ctrl){
  svc_createthread= ctrl ; 
}
...
void supervisor_c (unsigned int* ptr,unsigned int spsr){
  ...
  if (svc_createthread == 1)
  {
    thread_create((void*)ptr[0],(void*)ptr[1],ptr[2]);
    set_svc_createthread(0);
  }
  if (svc_endthread == 1)
  {
    set_svc_endthread(0);
  ...
  if (svc_recv == 1) {
    int i =buffercount(global_buffer);
    if (i == 0)
    {
      ptr[13]-=8;                                     //springt zur stelle wo erneut flag gesetzt wird für weiteren call
      maindequeue(ptr);

    }
    else{
      char x  = dequeue();                            //holt char aus buffer
      ptr[0] = (unsigned int)x; 
    }
    set_svc_recv(0);
  }
  if (svc_sleep == 1)
  {
    ...
    unsigned int time = ptr[0];
    set_svc_sleep(0);
    setcomparevalue_tcb((getclovalue() + time));
    sleepdequeue(ptr);
 }
  if (svc_tra == 1)
  {
...
void irq_extra_handler(unsigned int * ptr,unsigned int spsr){
  ...
  else if (x == 0) //sysstimer channel 1
  {
    switch_thread(ptr,0);                       //scheduling
    increase_compare_value(1,TIMER_INTERVAL);
    reset_interrupt(1);
  }
  else if (x == 3)//sysstimer channel 3
  {

    reset_interrupt(3);
    sleepenqueue(ptr);                         //dequeue aus svcwaiting queue und enqueue in runqueue
